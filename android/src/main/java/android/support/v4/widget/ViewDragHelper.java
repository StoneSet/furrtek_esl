...
public class ViewDragHelper {
    ...
    private int computeAxisDuration(int i, int i2, int i3) {
        ...
        i4 = Math.abs(i2);
        return Math.min(i4 > 0 ? Math.round(Math.abs(distanceInfluenceForSnapDuration / ((float) i4)) * 1000.0f) * 4 : (int) (256.0f * ((((float) Math.abs(i)) / ((float) i3)) + 1.0f)), MAX_SETTLE_DURATION);
    }
    ...
    private int getEdgesTouched(int i, int i2) {
        int i3 = 0;
        if (i < this.mEdgeSize + this.mParentView.getLeft()) {
            i3 = 1;
        }
